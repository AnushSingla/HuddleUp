...
import { UserPlus, Check, X, BellOff, MessageCircle } from 'lucide-react';
...
function getResourceLink(resource) {
  ...
  if (resource.resourceType === 'video' && resource.resourceId) return `/explore?video=${resource.resourceId}`;
  if (resource.resourceType === 'post' && resource.resourceId) return `/posts?post=${resource.resourceId}`;
  return null;
}
...
export default function NotificationDropdown({
  ...
  const totalNew = friendRequests.length + activityUnreadCount;

  if (!isOpen) return null;

  const handleActivityClick = (n) => {
    if (!n.isRead && onMarkAsRead) onMarkAsRead(n._id);
    const link = getResourceLink(n.resource);
    if (link) {
      onClose();
      navigate(link);
    }
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="max-h-[420px] overflow-y-auto custom-scrollbar">
          {/* Activity (comment / reaction notifications) */}
          {activityNotifications.length > 0 && (
            <div className="border-b border-zinc-100 dark:border-zinc-800">
              <div className="px-4 py-2 flex items-center justify-between bg-zinc-50/30 dark:bg-zinc-800/10">
                ...
                {activityUnreadCount > 0 && onMarkAllAsRead && (
                  <button
                    type="button"
              ...
              <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {activityNotifications.map((n) => (
                  <button
                    ...
                    className="w-full text-left p-4 hover:bg-emerald-500/5 dark:hover:bg-emerald-500/10 transition-colors flex items-start gap-3"
                  >
                    <div
                      className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold ${
                        n.isRead ? 'bg-zinc-300 dark:bg-zinc-600' : 'bg-gradient-to-br from-emerald-400 to-indigo-500'
                      }`}
                    >
                      {n.sender?.username?.charAt(0)?.toUpperCase() || '?'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-semibold ${n.isRead ? 'text-zinc-500 dark:text-zinc-400' : 'text-zinc-900 dark:text-zinc-100'}`}>
                        {n.message}
                      </p>
          ...
          {/* Friend requests */}
          <div>
            <div className="px-4 py-2 flex items-center gap-1 bg-zinc-50/30 dark:bg-zinc-800/10">
              <span className="text-[10px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider flex items-center gap-1">
                <UserPlus className="w-3 h-3" />
                Requests
              </span>
              {friendRequests.length > 0 && (
                <span className="text-[10px] font-bold text-emerald-500">{friendRequests.length}</span>
              )}
            </div>
            {loading ? (
              ...
              <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {friendRequests.map((req) => (
                  <div
                    key={req._id}
                    className="p-4 hover:bg-emerald-500/5 dark:hover:bg-emerald-500/10 transition-colors group"
                  >
              ...
              activityNotifications.length === 0 && (
                <div className="p-16 flex flex-col items-center justify-center text-center">
                  ...
                  <h4 className="text-base font-extrabold text-zinc-900 dark:text-zinc-100 tracking-tight">
                    Crickets here...
                  </h4>
                  <p className="text-xs text-zinc-500 dark:text-zinc-500 mt-2 font-bold uppercase tracking-widest leading-relaxed">
                    No new requests or activity
                  </p>
                </div>
              )
            )}
